...
const Header = () => {
    ...
    return (
        <>
            {
                location.pathname !== '/auth' && isMobile && <header className='fixed-top bg--secondary' >
                    <nav>
                        <Container maxWidth="xxl" className='d-flex justify-content-between align-items-center'>
                            <Box display={'flex'} gap={2} alignItems={'center'}>
                                <IconButton onClick={() => setMenuOpen(!menuOpen)}><Menu className='text-white' /></IconButton>
                            ...
                            </Box>
                            {
                                ...
                                </Popover> </> : <Button className='bg-white text-dark fw-semibold' onClick={() => navigate('/auth')}>Login</Button>
                            }
                            {
